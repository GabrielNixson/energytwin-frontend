...
const Auth = () => {
  ...
  const handleSSOLogin = (provider: 'google' | 'microsoft' | 'github') => {
    const baseUrl = import.meta.env.VITE_API_URL || 'http://192.168.21.114:3000';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError('');
    setLoading(true);
    setShowPassword(false); // Revert to password type for browser password manager detection
    try {
      if (isLogin) {
        ...
        if (res.success && res.data) {
          ...
          // Defer authentication store update slightly to allow Chrome password manager to register form submission
          setTimeout(() => {
            login(res.data.user || res.data, token);
            console.log("Login store!", res);
            setLoading(false);
          }, 500);
        } else {
